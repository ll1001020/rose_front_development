    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:varieties:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:varieties:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:varieties:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:varieties:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listVarieties, getVarieties, delVarieties, addVarieties, updateVarieties } from "@/api/system/varieties";
  ...
  methods: {
    ...
    getList() {
      ...
      listVarieties(this.queryParams).then(response => {
        this.varietiesList = response.rows;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除品种编号为"' + ids + '"的数据项？').then(function() {
        return delVarieties(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/varieties/export', {
        ...this.queryParams
